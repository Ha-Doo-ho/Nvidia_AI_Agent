# save를 하였으니, load를 해 보자. 
# 22_1을 카피 하였다. 

import tensorflow as tf 
from keras.models import Sequential, load_model #이것도 모델이기 때문에, load_model이다. 클래스가 아닌, 함수다. 
...
# model.summary()
# model.save("./_save/kerass22_1_save_model.keras") #가중치를 저장해보자. 확장자는 보통 상관은 없는데, .h5를 하면 된다. 요즘은 .keras도 있다고 한다.  가중치가 들어있긴 한데, 초기 랜덤값이 있다. 그래도 모델이 저장되어 있다. 
# . 붙이는 것은 상대경로 방식이다. 

model = load_model("./_save/kerass22_1_save_model.keras")
model.summary() #해보면 이전에 한 파라미터가 그대로 붙어온다.  --> 대신 초기 가중치로 되어 있다. 그래도 남의 모델을 그대로 가져올 수 있다는 것에서 굉장히 이득이라고 볼 수 있다. 
#가중치는 "초기 가중치이다." 우리는 모델만 가져온 것이다. 
