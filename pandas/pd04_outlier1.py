...
def outlier(data): 
    quantile_1, q2, quantile_3 = np.percentile(data, [25, 50, 75]) #25% 지점, 50%지점, 75%지점. 즉 1분위, 2분위, 3분위 값을 받겠다는 의미이다. 
    print("1사분위 : ", quantile_1)  # 4.0   1사분위 값
    print("q2 : ", q2)               # 7.0   중위값(2 사분위 값)
...
plt.axhline(up, color='pink', label='lower bound')
plt.legend()
plt.show()
