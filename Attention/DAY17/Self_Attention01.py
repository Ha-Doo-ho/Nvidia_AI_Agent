...
앞서 배운 LSTM이나 양방향(Bidirectional) LSTM은 문장을 앞에서부터 뒤로, 혹은 양쪽에서 가운데로 '순서대로' 읽었다.
...
4. 결론
결과적으로 '그것(it)'이라는 단어는 단순한 대명사가 아니라, 문장 안에서 관계를 맺고 있는 '동물'의 의미를 듬뿍 머금은 풍부한 문맥적 벡터로 재탄생한다. 
...
# 오히려 더 간단해 보일 수도 있다. 
inputs = Input(shape=(200, )) #입력층: 모양을 정의하고 변수(inputs)에 할당한다. 
